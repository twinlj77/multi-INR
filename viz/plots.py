...
def plot_point_cloud_batch_INR(point_clouds, ncols=4, save_fig=''):
    """Plots batches of voxels.
    ...
    Args:
        ...
        ncols (int): Number of columns in grid of images.
    """
    batch_size, _, point_clouds_size, _, _ = point_clouds.shape
    nrows = int((batch_size - 0.5) / ncols) + 1
    fig = plt.figure()

    # Permutation to get better angle of chair data
    point_clouds = point_clouds.permute(0, 1, 2, 4, 3)

    for i in range(batch_size):
        ax = fig.add_subplot(nrows, ncols, i + 1, projection='3d')

        # Non zero voxels define coordinates of visible points
        coords = point_clouds[i, 0].nonzero(as_tuple=False)
        ax.scatter(coords[:, 0], coords[:, 1], coords[:, 2], s=1)

        # Set limits to size of voxel grid
        ax.set_xlim(0, point_clouds_size - 1)
        ax.set_ylim(0, point_clouds_size - 1)
        ax.set_zlim(0, point_clouds_size - 1)
